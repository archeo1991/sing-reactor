...
1. **元数据与分 P**：根据当前分 P 标题、视频总标题、描述和分类推断歌名、歌手、语言及 Live/Cover 等版本信息；媒体和识别缓存按 BVID 与分 P 隔离。
2. **统一候选召回**：B站字幕、网易云、LRCLIB 和内置歌词都只作为候选，不再按返回顺序直接采用。B站字幕同样必须经过当前视频的 Whisper 音频验证。
3. **序列融合与候选竞争**：用完整 ASR 的词级或 segment 时间执行全局单调对齐，并综合行覆盖、字符覆盖、平均文本相似度、时间跨度、元数据和来源质量选出最佳候选。
4. **翻唱与现场门槛**：允许当前演唱者与歌词来源歌手不同，但必须达到更高的音频置信度、覆盖率和锚点数量；弱证据候选不能进入原始 LRC 兼容兜底。
5. **同步 LRC fallback**：完整序列融合拒绝时，同步 LRC 可尝试三段采样固定偏移或线性校准；纯文本候选没有可靠时间轴时不会直接采用。
6. **最终回退**：没有可靠在线歌词时返回 Whisper 原始文本和时间轴；Whisper 也没有结果时才继续视频字幕识别/OCR。配置、来源权重和流水线版本位于 `server/lyrics_config.json`。
